...
import { 
  ...
  Lightbulb, Palette, Rocket, BadgeCheck, Users, HelpCircle, Save, Check, Sparkles, ExternalLink
} from "lucide-react";
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto pb-32">
      <ModuleHeader 
        ...
        title="MVP & Adoption Readiness Workshop"
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 min-w-0">
          <AnimatePresence mode="wait">
            ...
            {step === 1 && (
              <motion.div key="s1" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} className="bg-white p-8 md:p-10 shadow-lg border-t-[4px] border-[#022f42] rounded-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8 mb-8">
                  <button onClick={() => setData({...data, status: "Concept"})} className={`p-6 border-2 rounded-sm text-left transition-all ${data.status === "Concept" ? "border-[#022f42] bg-gray-50" : "border-gray-100 hover:border-gray-300"}`}>
                    <Lightbulb className={`w-8 h-8 mb-3 ${data.status === "Concept" ? "text-[#022f42]" : "text-gray-300"}`} />
                  ...
                  <button onClick={() => setData({...data, status: "Prototype"})} className={`p-6 border-2 rounded-sm text-left transition-all ${data.status === "Prototype" ? "border-[#022f42] bg-gray-50" : "border-gray-100 hover:border-gray-300"}`}>
                    <Palette className={`w-8 h-8 mb-3 ${data.status === "Prototype" ? "text-[#022f42]" : "text-gray-300"}`} />
                  ...
                  <button onClick={() => setData({...data, status: "MVP"})} className={`p-6 border-2 rounded-sm text-left transition-all ${data.status === "MVP" ? "border-[#022f42] bg-gray-50" : "border-gray-100 hover:border-gray-300"}`}>
                    <Rocket className={`w-8 h-8 mb-3 ${data.status === "MVP" ? "text-[#022f42]" : "text-gray-300"}`} />
                  ...
                  <button onClick={() => setData({...data, status: "Live"})} className={`p-6 border-2 rounded-sm text-left transition-all ${data.status === "Live" ? "border-[#022f42] bg-gray-50" : "border-gray-100 hover:border-gray-300"}`}>
                    <BadgeCheck className={`w-8 h-8 mb-3 ${data.status === "Live" ? "text-[#022f42]" : "text-gray-300"}`} />
              ...
              <motion.div key="s2a" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} className="bg-white p-8 md:p-10 shadow-lg border-t-[4px] border-[#022f42] rounded-sm">
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <textarea value={data.builtSoFar} onChange={e=>setData({...data, builtSoFar: e.target.value})} maxLength={300} placeholder="e.g. Figma prototype finished. Frontend 40% coded. Database mapped." className="w-full p-4 border-2 border-gray-200 rounded-sm outline-none focus:border-[#ffd800] min-h-[100px] text-sm font-medium"/>
                  </div>
                  
                  <div className="flex flex-col md:flex-row gap-4">
                    <div className="w-full md:w-1/3">
                      <label className="block text-sm font-bold text-[#022f42] mb-2">Timeline to MVP</label>
                      <input type="month" value={data.timeline} onChange={e=>setData({...data, timeline: e.target.value})} className="w-full p-4 border-2 border-gray-200 rounded-sm outline-none focus:border-[#ffd800] text-sm font-medium"/>
                    </div>
                    <div className="w-full md:w-2/3">
                      ...
                      <input type="text" value={data.risks} onChange={e=>setData({...data, risks: e.target.value})} maxLength={200} placeholder="e.g. Waiting on partner API access..." className="w-full p-4 border-2 border-gray-200 rounded-sm outline-none focus:border-[#ffd800] text-sm font-medium"/>
                    </div>
                  </div>
                    ...
                    <div className="space-y-2 mb-4">
                      {["Internal team testing", "Beta waitlist", "Paid pilot", "Public beta", "No formal testing plan"].map(plan => (
                        <label key={plan} className="flex items-center gap-3 cursor-pointer">
                          <input type="checkbox" checked={data.testingPlan.includes(plan)} onChange={() => handleCheckbox("testingPlan", plan, "No formal testing plan")} className="w-5 h-5 accent-[#022f42] rounded-sm" />
                  ...
                  <table className="w-full text-sm text-left">
                    ...
                    <tbody>
                      <tr className="border-b">
                        <td className="px-5 py-4 font-bold text-[#1e4a62]">Total registered users</td>
                        <td className="px-5 py-2 border-l border-gray-100 bg-white"><input type="number" value={data.metrics.registered} onChange={e=>setData({...data, metrics: {...data.metrics, registered: e.target.value}})} className="w-full p-2 outline-none text-center font-bold focus:border-b-2 border-[#ffd800]" placeholder="0"/></td>
                      </tr>
                      <tr className="border-b">
                        <td className="px-5 py-4 font-bold text-[#1e4a62]">30-Day Active Users</td>
                        <td className="px-5 py-2 border-l border-gray-100 bg-white"><input type="number" value={data.metrics.active} onChange={e=>setData({...data, metrics: {...data.metrics, active: e.target.value}})} className="w-full p-2 outline-none text-center font-bold focus:border-b-2 border-[#ffd800]" placeholder="0"/></td>
                      </tr>
                      <tr className="border-b bg-[#fffdf0]">
                        <td className="px-5 py-4 font-bold text-[#1e4a62]">Paying Customers</td>
                        <td className="px-5 py-2 border-l border-gray-100"><input type="number" value={data.metrics.paying} onChange={e=>setData({...data, metrics: {...data.metrics, paying: e.target.value}})} className="w-full p-2 outline-none bg-transparent text-center font-black text-[#022f42] focus:border-b-2 border-[#ffd800]" placeholder="0"/></td>
                      </tr>
                    </tbody>
                  </table>
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-4">
                      {["In-app widget", "User interviews", "NPS Surveys", "Support tickets", "App Store reviews", "No formal feedback"].map(fb => (
                        <label key={fb} className="flex items-center gap-2 cursor-pointer">
                          <input type="checkbox" checked={data.feedbackMethods.includes(fb)} onChange={() => handleCheckbox("feedbackMethods", fb, "No formal feedback")} className="w-4 h-4 accent-[#022f42] rounded-sm" />
            ...
            {step === 3 && (
              <motion.div key="s3" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} className="bg-white p-8 md:p-10 shadow-lg border-t-[4px] border-[#022f42] rounded-sm">
                  ...
                  <div className="space-y-3 mb-4">
                    {["Customer interviews confirming need", "Beta users provided feedback", "Paid specifically for this problem", "Quantifiable ROI (time/money)", "No validation yet"].map(ev => (
                      <label key={ev} className="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" checked={data.validationParams.includes(ev)} onChange={() => handleCheckbox("validationParams", ev, "No validation yet")} className="w-5 h-5 accent-[#022f42] rounded-sm" />
                ...
                <div className="bg-[#f2f6fa] border border-gray-200 p-6 rounded-sm mb-6">
                  <label className="block text-sm font-bold text-[#022f42] mb-4">
                    If you stopped offering your product tomorrow, what would they do?
                  </label>
                  <div className="space-y-3">
                    {["switchable", "annoying", "trouble"].map(p => (
                      <button key={p} onClick={() => setData({...data, painkiller: p})} className={`w-full p-4 border-2 rounded-sm text-left flex items-center gap-4 transition-colors ${data.painkiller === p ? 'border-[#022f42] bg-white shadow-md' : 'border-gray-100 bg-white/50 hover:bg-white'}`}>
                        <div className="font-bold text-sm text-[#022f42] capitalize">{p === 'trouble' ? 'Operational Trouble (Painkiller)' : p === 'annoying' ? 'Annoyed (Vitamin)' : 'Switch Elsewhere (Me-too)'}</div>
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <motion.div key="s4" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} className="bg-white p-8 md:p-10 shadow-lg border-t-[4px] border-emerald-500 rounded-sm">
                ...
                <div className="flex flex-col items-center justify-center mb-10">
                  <div className="relative w-48 h-48 flex items-center justify-center">
                    <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                      ...
                      <path className={`${totalScore >= 80 ? 'text-emerald-500' : 'text-[#ffd800]'} transition-all duration-1000`} strokeDasharray={`${totalScore}, 100`} d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" strokeWidth="3" stroke="currentColor"/>
                    </svg>
                ...
                <div className="border border-gray-100 rounded-sm overflow-hidden mb-6">
                  <table className="w-full text-sm">
                    <tbody>
                      <tr className="border-b"><td className="p-3 font-bold text-[#1e4a62] bg-gray-50">Product Status</td><td className="p-3 text-right font-black">{scoreExistence}/40</td></tr>
                      <tr className="border-b"><td className="p-3 font-bold text-[#1e4a62]">Traction Metrics</td><td className="p-3 text-right font-black">{scoreTraction}/15</td></tr>
                      <tr className="border-b"><td className="p-3 font-bold text-[#1e4a62] bg-gray-50">Validation Depth</td><td className="p-3 text-right font-black">{scoreValidation}/20</td></tr>
                      <tr><td className="p-3 font-bold text-[#1e4a62]">Painkiller Power</td><td className="p-3 text-right font-black">{scorePainkiller}/15</td></tr>
                    </tbody>
                  </table>
                </div>
              </motion.div>
            ...
            {step === 5 && (
              <motion.div key="s5" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-8 md:p-10 shadow-lg border-t-[4px] border-[#ffd800] rounded-sm">
                ...
                <div className="flex justify-center">
                  <button onClick={handleSaveAndContinue} className={`px-12 py-5 font-black uppercase tracking-widest transition-all rounded-sm flex items-center gap-2 shadow-lg ${savedSuccess ? 'bg-green-500 text-white' : 'bg-[#ffd800] hover:bg-[#ffe24d] text-[#022f42]'}`}>
                    {savedSuccess ? <Check className="w-5 h-5"/> : <Save className="w-5 h-5"/>} {savedSuccess ? 'Saved Readiness' : 'Save & Continue'}
                  </button>
                </div>
              </motion.div>
            )}
            ...
            <div className="space-y-4">
              <div className="bg-white/10 p-4 rounded-sm border border-white/10">
                <p className="text-sm leading-relaxed text-blue-50 font-medium">
                  {step === 1 ? (aiFlags.step1 || "A 'Concept' is a bet on the founder. A 'Live Product' is a bet on the market.") : 
                   step === 2 ? (aiFlags.step2 || "Paying customers are the ultimate validation. Free users are 'interest', not 'demand'.") :
                   step === 3 ? (aiFlags.step3 || "The 'Painkeeper' vs 'Vitamin' test is how VCs filter 90% of deals.") :
                   "Your product readiness score determines your valuation multiple."}
                </p>
              </div>
              ...
              <div className="group">
                <Link 
                  href="/academy/the-product-model-fit" 
                ...
                >
                  <span>Education: Product-Model Fit →</span>
                  <ExternalLink className="w-3 h-3 shrink-0" />
                </Link>
                <p className="text-[10px] text-gray-400 mt-1 opacity-0 group-hover:opacity-100 transition-opacity">Academy: Engineering PMF</p>
              </div>
            </div>
          </div>

          <div className="bg-white p-6 border border-gray-100 shadow-sm rounded-sm">
            <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 mb-3">Readiness Gauge</h4>
            <div className="flex items-center gap-3">
              <div className="text-2xl font-black text-[#022f42]">{totalScore}%</div>
              <div className="flex-1 h-3 bg-gray-100 rounded-full overflow-hidden">
                <div className={`h-full transition-all duration-500 ${totalScore >= 80 ? 'bg-emerald-500' : 'bg-[#ffd800]'}`} style={{width: `${totalScore}%`}} />
              </div>
            </div>
          </div>
        </div>
