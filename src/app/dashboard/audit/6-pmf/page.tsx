              ...
              <motion.div key="s2" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} className="bg-white p-8 md:p-10 shadow-lg border-t-[4px] border-[#022f42] rounded-sm">
                ...
                <div className="space-y-10">
                  ...
                  <div className="p-6 bg-indigo-50 border border-indigo-100 rounded-sm">
                    <div className="flex justify-between items-end mb-4">
                       <div>
                         <label className="text-xs font-black uppercase text-indigo-400 block mb-1">Viral Coefficient (K-Factor)</label>
                         <span className="text-[10px] font-bold text-indigo-400 block max-w-[200px] leading-tight">Avg. number of new users generated by each existing user.</span>
                       </div>
                      <span className="text-3xl font-black text-[#022f42]">{data.viralCoefficient}</span>
                    </div>
                    <input type="range" min="0" max="2" step="0.1" value={data.viralCoefficient} onChange={e=>setData({...data, viralCoefficient: parseFloat(e.target.value)})} className="w-full accent-indigo-600" />
                    <div className="mt-4 p-3 bg-white/50 rounded-sm text-[10px] text-[#1e4a62] font-bold border border-indigo-100 flex items-start gap-2">
                      <HelpCircle className="w-3 h-3 text-indigo-400 shrink-0 mt-0.5" />
                      <span>Calculation: (Avg. Invites Sent per User) × (Conversion Rate). A K-factor {'>'} 1.0 means exponential organic growth. B2B typically ranges 0.1 – 0.3.</span>
                    </div>
                  </div>
                </div>
              </motion.div>
